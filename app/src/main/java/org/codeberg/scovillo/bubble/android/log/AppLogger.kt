package org.codeberg.scovillo.bubble.android.log
...
object AppLogger {
    ...
    private fun log(
        ...
        when (level) {
            LogLevel.VERBOSE -> if (throwable != null) Log.v(
                tag,
                prefixedMessage,
                throwable
            ) else Log.v(tag, prefixedMessage)

            LogLevel.DEBUG -> if (throwable != null) Log.d(
                tag,
                prefixedMessage,
                throwable
            ) else Log.d(tag, prefixedMessage)

            LogLevel.INFO -> if (throwable != null) Log.i(
                tag,
                prefixedMessage,
                throwable
            ) else Log.i(tag, prefixedMessage)

            LogLevel.WARN -> if (throwable != null) Log.w(
                tag,
                prefixedMessage,
                throwable
            ) else Log.w(tag, prefixedMessage)

            LogLevel.ERROR -> if (throwable != null) Log.e(
                tag,
                prefixedMessage,
                throwable
            ) else Log.e(tag, prefixedMessage)
        }
    }
}
