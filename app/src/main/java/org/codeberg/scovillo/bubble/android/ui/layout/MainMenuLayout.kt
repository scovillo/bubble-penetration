...
import org.codeberg.scovillo.bubble.android.ui.render.MenuBubbleScene
...
class MainMenuLayout(private val mainActivity: MainActivity, private val musicPlayer: MusicPlayer) {
    ...
    fun hide() {
        ...
        val glSurfaceViewHolder =
            mainActivity.findViewById<View?>(R.id.menuGLSurfaceViewHolder) as? FrameLayout ?: return
        glSurfaceViewHolder.removeAllViews()
    }

    private fun createStartButtonPulse(button: Button): AnimatorSet {
        fun pulse(property: String, from: Float, to: Float) =
            ObjectAnimator.ofFloat(button, property, from, to).apply {
                duration = 900
    ...
    private fun loadCurrentChampionAsync() {
        ...
        THREAD_POOL.execute {
            try {
                ...
                val jsonArray =
                    highscoreRequest[8000, TimeUnit.MILLISECONDS].getJSONArray("highscores")
