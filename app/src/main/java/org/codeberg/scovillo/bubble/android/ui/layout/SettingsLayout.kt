...
class SettingsLayout(
    ...
    fun show(backendUrl: String? = null) {
        ...
        mainActivity.findViewById<CheckBox>(R.id.effects_box).isChecked =
            !settingsModel.areSoundEffectsMuted
        mainActivity.findViewById<CheckBox>(R.id.vibration_box).isChecked =
            settingsModel.isVibrationEnabled
        ...
        mainActivity.findViewById<CheckBox>(R.id.vibration_box)
            .setOnClickListener { setVibration(it) }
        onlineLeaderboardBox.setOnClickListener { setOnlineLeaderboard(it) }
        mainActivity.findViewById<Button>(R.id.test_connection_button)
            .setOnClickListener { testBackendConnection() }
        mainActivity.findViewById<Button>(R.id.reset_backend_url_button)
            .setOnClickListener { resetBackendUrl() }
        mainActivity.findViewById<Button>(R.id.back_to_menu_button)
            .setOnClickListener { backToMenu() }
    }
    ...
    private fun setOnlineLeaderboard(view: View) {
        ...
        if (view.isChecked) {
            mainActivity.registerSelectedUserIfNeeded()
        }
    }
    ...
    private fun backendUrlToTest(): String? {
        val value =
            mainActivity.findViewById<EditText>(R.id.backend_url_field)?.text?.toString()?.trim()
                ?: return null
        if (value.isBlank() || !(value.startsWith("https://") || value.startsWith("http://"))) {
            ...
            Toast.makeText(
                mainActivity,
                mainActivity.getString(R.string.invalid_backend_url),
                Toast.LENGTH_LONG
            ).show()
            return null
        }
    ...
    private fun resetBackendUrl() {
        ...
        mainActivity.findViewById<EditText>(R.id.backend_url_field)
            .setText(settingsModel.backendBaseUrl)
    }
